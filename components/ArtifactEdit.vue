    ...
    <div>
      <v-stepper v-model="step" non-linear>
        ...
        <v-stepper-content step="1">
          ...
          <v-form v-model="validStep1" ref="step1">
            <v-card class="mx-auto my-2">
              <v-card-title class="py-0 mt-2">
                Artifact Type -
                {{ artifact_local.type | titlecase }}</v-card-title
              >
              <!-- <v-row justify="space-around">
                ...
                >
                  <v-radio
                    ...
                    :value="type"
                    :on-icon="iconImage(type)"
                    :off-icon="iconImage(type)"
              ...
              <!-- <v-row justify="space-around">
                <v-chip-group v-model="artifact_local.type">
                  <v-chip
                    v-for="(type, index) in types"
                    :key="`typeicon${index}`"
                    :value="type"
              ...
              <v-row justify="space-around">
                ...
                >
                  <template v-slot:activator="{ on, attrs }">
                    <v-btn
                      :color="iconColor(type)"
                      @click="artifact_local.type = type"
        ...
        <v-stepper-content step="2">
          Please provide author information below
          ...
          <v-form v-model="validStep3">
            <v-card class="mx-auto my-2">
                ...
                >
                  ...
                  <template v-slot:default="artifactdialog">
                    <v-card>
                      <v-card-title>
                        <span class="text-h5"
                          >Search for Related Artifacts</span
                        >
                      </v-card-title>
              ...
              <v-chip cols="12" class="ma-2" label>
                ...
                <v-icon @click="artifact_local.license = []" right
                  >mdi-close</v-icon
                >
              </v-chip>
              <v-divider class="mx-4"></v-divider>
                  ...
                  <v-list-group :value="true" no-action sub-group>
                    <template v-slot:activator>
                      <a
                        ...
                        >{{ f.url }}</a
                      >
                      &nbsp; (type: {{ f.filetype ? f.filetype : 'unknown' }},
                      size: {{ f.size ? convertSize(f.size) : 'unknown' }})
                    </template>
                    <v-list-item
                    ...
                    >
                      ...
                      >
                      &nbsp; (type: {{ fm.filetype ? fm.filetype : 'unknown' }},
                      size: {{ fm.size ? convertSize(fm.size) : 'unknown' }})
                    </v-list-item>
                  </v-list-group>
...
export default {
  ...
  methods: {
    async publish() {
      if (!this.validStep1 && !this.validStep2 && !this.validStep3) return
